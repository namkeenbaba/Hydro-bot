...
void  CapacitiveMoisture::loop() {
  ...
  if (m_loopSNSLAPSEINTERVAL == 0)
  {
    read();
    return;
  }
  unsigned long nowMillis = millis()  ;
  if (nowMillis < m_loopSNSLAPSEINTERVALPrevious) {
    m_loopSNSLAPSEINTERVALPrevious = 0; // overflow counter
  }
  if (nowMillis - m_loopSNSLAPSEINTERVALPrevious > m_loopSNSLAPSEINTERVAL)
  {
    m_loopSNSLAPSEINTERVALPrevious = nowMillis;
    read();
...
void     CapacitiveMoisture::setup(unsigned short Pin, unsigned long loopReadInterval, unsigned int CalibrateMin =CALIBRATEMIN, unsigned int CalibrateMax=CALIBRATEMAX)   {
  m_pin = Pin;
  pinMode(m_pin, INPUT);
  m_loopSNSLAPSEINTERVAL = loopReadInterval;
...
void     CapacitiveMoisture::setLoopReadInterval(unsigned long loopSNSLAPSEINTERVAL) {
  m_loopSNSLAPSEINTERVAL = loopSNSLAPSEINTERVAL;
}
