...
class CapacitiveMoisture {
  ...
  public:
    ...
    unsigned int read() ;
    void         setup(unsigned short Pin, unsigned long loopSNSLAPSEINTERVAL, unsigned int CalibrateMin , unsigned int CalibrateMax);
  ...
  private:
    ...
    unsigned long   m_loopSNSLAPSEINTERVAL = 0;        // if you call loop the sensor will read on this millis intervals only, 0=allways;
    unsigned long   m_loopSNSLAPSEINTERVALPrevious = 0; // is work variable for calculate loop fire read sensor
